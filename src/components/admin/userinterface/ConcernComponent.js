    ...
    var settings = {
      ...
      const showCategory=()=>{
        return category?.map((item)=>{
            return <div><paper  elevation={0}><Avatar  variant="square" alt="Cindy Baker" src={`${serverURL}/images/${item.picture}`} style={{width:'88%',height:'auto',zIndex:2,borderRadius:10,marginRight:'auto',marginLeft:'auto',height:'auto',zIndex:2, boxShadow:"0 0 60px #ecf0f1"}} /></paper>
            <span style={{display:'flex',justifyContent:'center',textAlign:'center',fontWeight:'bold'}}>{item.concernname}</span>
            </div>
