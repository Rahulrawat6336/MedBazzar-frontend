...
{   var productFromRedux=useSelector(state=>state.data)
    ...
    //    <div style={{width:'100%',display:'flex',justifyContent:'center',background:'#d3d3d324'}} >
        <div style={{width:'80%',padding:10,height:'auto'}} >
        <div style={{fontWeight:'bold',fontSize:18}} >{product?.productname}, {product?.weight} {product?.weighttype}</div>
        <div style={{fontWeight:'bold',fontSize:18}} >{product?.productsubname}</div>

        <div style={{fontWeight:'bold',fontSize:13}} ><a href="#">Be the First One to Review</a></div>
        <div style={{fontWeight:'bold',fontSize:25}} >&#8377; {product?.offerprice!=0?product?.offerprice:product?.price}</div>
        <span style={{fontWeight:'bold',fontSize:10}} >(incl. all Taxes)</span>
        <hr/>
        <div style={{fontWeight:'bold',fontSize:18,color:'grey'}} >
            <s>MRP:&#8377;150</s>
            <span style={{margin:5,color:'black'}} >{product?.offerprice!=0?<div>(save &#8377; {product?.price-product?.offerprice})</div>:<div></div>}</span>
        </div>
        ...
        <div style={{marginTop:20,width:'70%'}} >
            ...
            <Grid item xs={6}>
              <Button onClick={()=>navigate('/home')} fullWidth size="small" style={{background:'#00391c',width:'80%',color:'#fff',fontWeight:'bold',whiteSpace:'nowrap'}} >Continue Shopping</Button>  
              </Grid>
